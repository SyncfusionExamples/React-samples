...
gulp.task('plnkr-json', function() {
    ...
    var indHtml = fs.readFileSync('./src/common/plnk-template/index.html', 'utf8');
    ...
    indHtml = indHtml.replace(/{{:CDN_LINK}}/g, link);
    var comp;
    for(var i = 0; i < files.length; i++) {
        var plnkr = { 'index.css': '' };
        var ind = indHtml;
        var compName = files[i].split('/')[2];
        ...
        var header2 = compName.toLowerCase();
        ...
        var title = compName + ' · ' + className + ' · Essential JS 2 for React · Syncfusion';
        ...
        ind = ind.replace(/{{:action-description}}/, adescription).
        replace(/{{:description}}/, description);
        ind = ind.replace(/{{:title}}/g, title);
        ind = ind.replace(/{{:header1}}/g, className);
        ind = ind.replace(/{{:header2}}/g, header2);
        ind = ind.replace(/{{:header}}/g, compName);
